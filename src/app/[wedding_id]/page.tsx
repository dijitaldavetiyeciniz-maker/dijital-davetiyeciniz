...
export default async function WeddingPage({
  ...
  // DRAFT / PUBLISH (Yayın Durumu) Kontrolü
  // Public visitors can ONLY view published invitations via their published_snapshot
  const isPublished = wedding.is_published === true || (wedding.is_paid && wedding.is_published !== false);

  if (!isPublished && !isPreview) {
    return (
      <div className="min-h-screen bg-[#0a0a14] flex flex-col items-center justify-center p-6 text-center text-white font-sans selection:bg-rose-500/30">
        <div className="w-20 h-20 bg-rose-500/10 border border-rose-500/20 rounded-3xl flex items-center justify-center mb-6 text-rose-400 shadow-xl shadow-rose-500/10">
          <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 border border-amber-500/20 text-amber-300 uppercase tracking-wider mb-3">
          Taslak Aşamasında
        </div>
        <h1 className="text-3xl font-bold font-serif mb-3 text-white">Bu Davetiye Henüz Yayında Değil</h1>
        <p className="text-slate-400 text-sm max-w-md mb-8 leading-relaxed">
          Bu davetiye sayfası henüz tasarım aşamasındadır. Davetiye sahibi tarafından yayınlandığında bu adresten görüntülenebilecektir.
        </p>
        <p className="text-xs text-slate-500">Dijital Davetiyeciniz &copy; 2026</p>
      </div>
    );
  }

  // If public view of a published invitation, prioritize the published snapshot to isolate live visitors from ongoing draft edits
  const pubSnapshot = wedding.published_snapshot || wedding.custom_overrides?.published_snapshot;
  if (!isPreview && pubSnapshot) {
    Object.assign(wedding, pubSnapshot);
  }
