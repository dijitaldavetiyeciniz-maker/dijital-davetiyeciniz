...
export async function PUT(request: NextRequest, props: { params: Promise<{ group_id: string }> }) {
  try {
    ...
    // Önce grubu bul ve wedding_id'sini al. Service-role client ile (bkz.
    // guests/[guest_id]/route.ts'teki aynı desen): bu sorgu yetki
    // kararindan once calisiyor, RLS'e tabi client kullanilirsa yetkisiz
    // istekler "grup yok" (404) gibi gorunur - oysa dogrusu "401 yetkisiz".
    const lookupClient = createServerServiceRoleClient();
    const { data: group, error: groupError } = await lookupClient
      .from('guest_groups')
...
export async function DELETE(request: NextRequest, props: { params: Promise<{ group_id: string }> }) {
  try {
    ...
    // Ayni sebep: bkz. PUT fonksiyonundaki aciklama.
    const lookupClient = createServerServiceRoleClient();
    const { data: group, error: groupError } = await lookupClient
      .from('guest_groups')
      .select('id, wedding_id')
