...
components.forEach(comp => {
  const fileContent = `export function ${comp}Opening({ opened, initials, brideName, groomName, eventDate, introText, wedding }: any) {
  return (
...
`;
  const filePath = path.join(__dirname, "../src/components/invitation/openings", `${comp}Opening.tsx`);
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, fileContent);
    console.log(`Created ${comp}Opening.tsx`);
  }
});
