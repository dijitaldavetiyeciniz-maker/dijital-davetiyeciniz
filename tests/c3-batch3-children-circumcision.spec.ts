...
test.describe("C3-B3 - Children & Circumcision Templates E2E", () => {
  ...
  for (const theme of childrenThemes) {
    test(`Verify Children template: ${theme.id}`, async ({ page }) => {
    ...
    test(`Verify mobile flow for Children theme: ${theme.id}`, async ({ page }) => {
  ...
  test("Verify prefers-reduced-motion vertical stacking behavior", async ({ page }) => {
    ...
    const targetTheme = childrenThemes[0];
