...
test.describe("C3-B4 - Corporate & Experimental Templates E2E", () => {
  ...
  for (const theme of corporateThemes) {
    test(`Verify Corporate template: ${theme.id}`, async ({ page }) => {
    ...
    test(`Verify mobile flow for Corporate theme: ${theme.id}`, async ({ page }) => {
  ...
  test("Verify prefers-reduced-motion vertical stacking behavior", async ({ page }) => {
    ...
    const targetTheme = corporateThemes[0];
